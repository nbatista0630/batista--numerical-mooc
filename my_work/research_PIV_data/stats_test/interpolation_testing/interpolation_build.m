...
av_u=mean(u_d_cut,1);
av_v=mean(v_d_cut,1);
dlmwrite('av_u_B00001.txt',av_u)
dlmwrite('av_v_B00001.txt',av_v)
...
figure
hold on;
for k=93:a(2) %Ignore where mask does not match surface
    plot(coord,v_d(k,1:end))
end
hold off;
figure
hold on;
for l=93:a(2) %Ignore where mask does not match surface
