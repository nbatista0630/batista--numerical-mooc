...
[Xdata1,Ydata1,Udata1,Vdata1] = textread('B00010.txt','%f %f %f %f','headerlines',1);
...
index_o=textread('mask_index_r3_B00010.txt');
index_surf=index_o;
...
av_u=mean(u_d_cut,1);
av_v=mean(v_d_cut,1);
dlmwrite('av_u_B00010.txt',av_u)
dlmwrite('av_v_B00010.txt',av_v)
...
figure
hold on;
for k=104:a(2) %Ignore where mask does not match surface
    plot(coord,v_d(k,1:end))
end
hold off;
figure
hold on;
for l=104:a(2) %Ignore where mask does not match surface
